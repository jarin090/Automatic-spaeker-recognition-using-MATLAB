...
for i=1:numOfCentroids
    ...
    while(1==1)
        dis = jdistance(d, codebk);            % distance of each point to every code word
        [m,ind] = min(dis, [], 2);          % ind maps points in 'd' to closest centroid
        t = 0;
        lim = 2^i;
        for j=1:lim
            codebk(:, j) = mean(d(:, ind==j), 2);    % updating centroids to better mean values
            x = jdistance(d(:, ind==j), codebk(:, j));  % x is a cluster i.e vector of neighbouring ...
